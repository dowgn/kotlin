/*
 * LlmClient backed by Mistral AI's Chat Completions API (api.mistral.ai),
 * using only java.net.http.HttpClient (JDK) and the hand-rolled Json.kt —
 * same zero-dependency constraint as the other providers. Mistral's API
 * is OpenAI-compatible in shape, so this mirrors OpenAiHttpClient.
 */
...
class MistralHttpClient(
    ...
    private val model: String = "mistral-large-latest",
    private val tools: List<Tool>,
    private val systemPrompt: String = "You are a helpful assistant with access to tools.",
    private val baseUri: URI = URI.create("https://api.mistral.ai/v1/chat/completions"),
) : LlmClient {
    ...
    override suspend fun next(prompt: Prompt, history: List<AgentStep>): AgentStep {
        ...
        check(response.statusCode() in 200..299) {
            "Mistral call failed with HTTP ${response.statusCode()}: ${response.body()}"
        }
